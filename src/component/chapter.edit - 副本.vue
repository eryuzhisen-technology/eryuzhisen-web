...
<div class="c-artice-edit-wrap">
    <div class="c-artice-edit__create">
        <div class="create-top">
            ...
            <!-- 授权信息填写 -->
                <div v-if="control_id == 1" class="create-top-item create-top-input create-top-oldname cpm_form_input">
                ...
                <div v-if="control_id == 1" class="create-top-item create-top-input create-top-oldauthor cpm_form_input">
                ...
                <div v-if="control_id == 1" class="create-top-item create-top-input create-top-oldurl cpm_form_input">
                ...
                <div v-if="control_id == 1" class="create-top-item create-top-input create-top-oldimg">
            ...
            <!-- 标签 -->
                <div class="create-top-item create-top-tagsInput j-close-2">
                    <div v-if="!reg_tag" class="c-dialog__tip z-right">
                        <div class="tip-arrow"></div>
                        <div class="tip-text">{{reg_tag_con}}</div>
                    </div>
                    <div class="tag-wrap cpm_form_input" :class="{'z-limit': tags.length>3, 'z-error': tagsshow}" @click="tagsshowOpen">
                        <div class="tag-list">
                            <div class="tag-item" v-for="(item, index) in tagsObj" @click.stop="empty">
                                <input type="text" class="tag-item-text" v-model="tagsObj[index]" :style="{width: tagsObj[index].gblen()*7 + 22 + 'px'}" />
                                <em @click.stop="deleteTag(index)"></em>
                            </div>
                        </div>
                        <div class="tag-edit">多个标签用空格区分</div>
...
export default {
    ...
    methods: {
        ...
        tagsshowOpen (e){
            ...
            var node = e.currentTarget;
                node = $(node);
            $('.j-close-2').find('.tag-edit').hide();
            this.tags.push('');
            setTimeout( res=> {
                $('.j-close-2 .tag-item').last().find('input').focus();
            }, 40)
        },
        deleteTag (index){
            index = Number(index);
        ...
        tagsSelect (e){
            ...
            if (node.hasClass('z-select')) {
                for(var i = 0, len = this.tags.length; i < len;) {
                    if (tag == this.tags[i]) {
            ...
            if (!node.hasClass('z-select') && this.tags.length < 3) {
                this.tags.push(tag);
            }
            setTimeout( res=> {
                $('.j-close-2 .tag-item').last().find('input').focus();
    ...
    watch: {
        ...
        tagsshow (){
            ...
            if (!this.tags.length && !this.tagsshow) {
                $('.j-close-2').find('.tag-edit').show();
            }
        },
        tags (){
            if (this.tags.length == 0 && this.tagsshow) {
                this.tags.push('');
                setTimeout( res=> {
                    $('.j-close-2 .tag-item').last().find('input').focus();
                }, 40)
