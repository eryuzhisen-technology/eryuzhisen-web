...
<div class="c-author-list-wrap" :style="{'height': Math.ceil(1)*360 + 'px', 'width': (lists.length == 0 ? 720 : lists.length <= 5 ? lists.length*240 : 1200) + 'px'}">
    <a class="list-author__item" v-for="(data, index) in lists" :href="'author.work.html?user_id=' + data.uid">
        ...
        <div v-if="data.relation == 0 || data.relation == 2" class="list-author__btn list-author__btn-1 z-active" @click.stop.prevent="addFollow(data.uid)">关注</div>
        <div v-if="data.relation == 1" class="list-author__btn list-author__btn-2" @click.stop.prevent="delFollow(data.uid, index)">已关注</div>
        <div v-if="data.relation == 3" class="list-author__btn list-author__btn-3" @click.stop.prevent="addFollow(data.uid)">相互关注</div>
